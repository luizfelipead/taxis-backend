...
import com.taxis.backend.controller.DriverStatusController;
...
public class MongoDBDriverStatusRepository implements DriverStatusRepository{
	...
	public int upsert(DriverStatus ds) throws TaxisBackendCustomDBException {
		...
		BasicDBObject findDoc = new BasicDBObject(MongoDBConstants.ID_KEY, ds.getDriverId());
		int result = 0;
		try {
			BasicDBObject upsertDoc = new BasicDBObject(MongoDBConstants.ID_KEY, ds.getDriverId());
			if (ds.getLatitude() != null) {
				upsertDoc.append(MongoDBConstants.DRIVER_STATUS_LATITUDE_KEY, ds.getLatitude());
			}
			if (ds.getLongitude() != null) {
				upsertDoc.append(MongoDBConstants.DRIVER_STATUS_LONGITUDE_KEY, ds.getLongitude());
			}
			if(ds.isAvailable() != null) {
				upsertDoc.append(MongoDBConstants.DRIVER_STATUS_AVAILABLE_KEY, ds.isAvailable());
			}
			result = collection.update(findDoc, upsertDoc, true, false).getN();
	...
	public List<DriverStatus> findInArea(double swLat, double swLong,
			double neLat, double neLong, boolean availableOnly) throws TaxisBackendCustomDBException {
